...
async function xm(apis: api[], index: number, req: Request) {
  let { user, pass, step } = req.query;
  if (step == undefined || step == "") {
    step = getRandomInt(28000, 35000) + "";
  }
  ...
  if (
    ...
    apis[index].params.password = pass;
    if (+step <= 99999) {
      const result = await main(user as string, pass as string, step as string);
      // console.log(result);
      return {
        code: result.code,
  ...
  async function login(user: string, pass: string) {
    ...
    if (access != null && access != "") {
      ...
      return { login_token, user_id, code: 1, message: "登录成功" };
    } else {
      return {
        code: 0,
        message: "登录失败,请检查账号与密码的正确性或者联系管理员",
      };
    }
  }
  async function get_app_token(login_token: string) {
    let code = 0;
    let message = "";
    let app_token = "";
    await axios
      ...
      .then(({ data }) => {
        ...
        code = 1;
        message = "获取app_token成功";
      })
      .catch((error: AxiosError) => {
        code = 0;
        message = "获取app_token失败,请联系管理员 => " + error.message;
      });
  ...
  async function doit(
    ...
    let message = "";
    let code = 0;
    const data_file = path.join(__dirname, `./cache/${user}.json`);
    ...
    await axios
      ...
      .then(async ({ data }) => {
        if (data.message == "invalid token") {
          ...
          const { login_token, user_id, code, message } = await login(
            user,
            pass
          );
          if (code == 1) {
            const { code, app_token, message } = await get_app_token(
              login_token
            );
            if (code == 1) {
              doit(user, pass, app_token, user_id, step);
              return;
            } else {
  ...
  async function main(user: string, pass: string, step: string) {
    let result = {
      message: "默认消息",
      cache: false,
      code: 0,
    ...
    let USER_id = "";
    // 创建缓存
    fs.existsSync(path.join(__dirname, "./cache")) ||
      (await fs.promises.mkdir(path.join(__dirname, "./cache")));
    const data_file = path.join(__dirname, `./cache/${user}.json`);
    let usercache = { app_token: "", user_id: "" };

    if (fs.existsSync(data_file)) {
      result.cache = true;
      usercache = JSON.parse(fs.readFileSync(data_file, "utf8"));
      APP_token = usercache.app_token;
      ...
      if (code == 1) {
        const { code, message, app_token } = await get_app_token(login_token);
        if (code == 1) {
          APP_token = app_token;
          USER_id = user_id;
          await fs.promises.writeFile(
            data_file,
            JSON.stringify({ app_token: APP_token, user_id: USER_id })
          );
        } else {
          // 异常错误（获取app_token失败，按道理不该在这里出错）
    ...
    // doit
    result.message = (await doit(user, pass, APP_token, USER_id, step)).message;
    result.code = (await doit(user, pass, APP_token, USER_id, step)).code;
